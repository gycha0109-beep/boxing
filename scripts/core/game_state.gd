...
func new_career(boxer_name: String, trait_id: String = "") -> void:
    var trait := _pick_trait(trait_id)
    var base_boxer := {
        "name": boxer_name,
        "power": 50,
        "speed": 50,
    ...
    for stat in TRAINABLE_STATS:
        base_boxer[stat] = clamp(int(base_boxer[stat]) + int(trait.get("stat_bonus", {}).get(stat, 0)), 1, 100)
...
func apply_camp_action(action: Dictionary) -> Dictionary:
    ...
    var growth_multiplier := float(boxer.modifiers.get("growth_multiplier", 1.0))
    for stat in TRAINABLE_STATS:
        if effects.has(stat):
            var raw_gain := int(effects[stat])
            var gain := raw_gain
            if raw_gain > 0:
                gain = max(1, int(round(float(raw_gain) * growth_multiplier)))
            boxer[stat] = clamp(int(boxer[stat]) + gain, 1, 100)

    var fatigue_delta := int(effects.get("fatigue", 0))
    ...
    if float(action.get("risk", 0.0)) > 0.0:
        var risk := float(career_balance.injury.base_training_chance) + float(action.risk)
...
func get_fight_offers(all_opponents: Array) -> Array:
    var current_tier := str(state.career.tier)
    var current_index := _tier_index(current_tier)
    var same: Array = []
    var lower: Array = []
    var higher: Array = []
    for opponent in all_opponents:
        var index := _tier_index(str(opponent.tier))
...
func apply_fight_result(result: String, opponent: Dictionary, snapshot: Dictionary = {}) -> void:
    ...
    var career: Dictionary = state.career
    var boxer: Dictionary = state.boxer
    var purse_multiplier := float(state.last_weigh_in.get("purse_multiplier", 1.0))
    purse_multiplier *= float(boxer.modifiers.get("purse_multiplier", 1.0))
    var gross_purse := int(round(float(opponent.purse) * purse_multiplier))
    var purse := int(round(float(gross_purse) * float(career_balance.economy.purse_net_multiplier)))
    var cycle_cost := int(career_balance.economy.cycle_cost_by_tier.get(str(opponent.tier), 0))
    ...
    var fight_fatigue := 14
    var fight_health_loss := 1
    if result == "LOSS_KO":
        fight_fatigue += 12
        fight_health_loss += 8
    elif result == "WIN_KO":
    ...
    var reputation_multiplier := float(boxer.modifiers.get("reputation_multiplier", 1.0))
...
func age_text() -> String:
    var months := int(state.career.age_months)
    return "%d세 %d개월" % [int(months / 12), months % 12]

func tier_label() -> String:
    var tier_id := str(state.career.tier)
...
func _resolve_weigh_in() -> Dictionary:
    var boxer: Dictionary = state.boxer
    var limit := float(career_balance.weight_class.limit_kg)
    var over := float(boxer.weight_kg) - limit
    if over <= 0.0:
    ...
    var multiplier := float(career_balance.weigh_in.miss_purse_multiplier)
...
func _roll_fight_injury(result: String) -> void:
    var boxer: Dictionary = state.boxer
    var chance := float(career_balance.injury.base_fight_chance)
    if result == "LOSS_KO":
...
func _pick_event() -> Dictionary:
    ...
    for event in events:
        ...
        var prefix := str(requires.get("last_result_prefix", ""))
...
func _begin_next_cycle() -> void:
    var boxer: Dictionary = state.boxer
    var recovery_bonus := int(boxer.modifiers.get("recovery_bonus", 0))
