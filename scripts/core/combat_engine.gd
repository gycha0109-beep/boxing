...
var player_round_score := 0.0
var opponent_round_score := 0.0
var cards: Array = []
var finished := false
var result := ""
...
func start(player_stats: Dictionary, opponent_data: Dictionary) -> void:
    ...
    round_no = 1
    exchange_no = 0
    var player_health := float(player.get("health", 100))
    var player_fatigue := float(player.get("fatigue", 0))
...
func resolve_exchange(player_action: String) -> Dictionary:
    if finished:
        return snapshot()
    var opponent_action := _choose_opponent_action()
    exchange_no += 1
...
func _perform(is_player: bool, action_id: String, target_action: String) -> void:
    ...
    var actor := player if is_player else opponent.stats
    var target := opponent.stats if is_player else player
    var stamina := player_stamina if is_player else opponent_stamina
    ...
    var cost := float(action.stamina) * (1.10 - float(actor.conditioning) / 500.0)
    ...
    if rng.randf() > accuracy:
        log.append(("나" if is_player else opponent.name) + " " + str(action.label) + " 빗나감")
        return

    var damage := float(action.damage)
    ...
    log.append(("나" if is_player else opponent.name) + " " + str(action.label) + " 적중 %.1f" % damage)
    _check_ko(is_player, damage)

func _check_ko(attacker_is_player: bool, damage: float) -> void:
    var target_hp := opponent_hp if attacker_is_player else player_hp
    var target := opponent.stats if attacker_is_player else player
    if target_hp <= 0.0:
        finished = true
    ...
    if damage < 10.0:
        return
    var chin := (float(target.defense) + float(target.conditioning)) * 0.5
    var chance := float(balance.fight.ko_base_threshold)
