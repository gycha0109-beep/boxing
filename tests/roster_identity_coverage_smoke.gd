...
func _run() -> void:
    ...
    _verify_diego_source_cleanliness()
    _finish()

func _verify_diego_source_cleanliness() -> void:
    var path := VisualAssetCatalog.opponent_identity_path_for_name(DIEGO_NAME)
    _check(not path.is_empty() and ResourceLoader.exists(path), "Diego identity source is unavailable for cleanliness smoke")
    ...
    _check(source_image.get_width() == 120 and source_image.get_height() == 280, "Diego identity source must remain 120x280")
    _check(_count_diego_guide_pixels(source_image) == 0, "Diego packaged source still contains red/green construction guide pixels")
    _check(_count_diego_face_artifact_pixels(source_image) <= 8, "Diego packaged source still contains the dark facial square artifact")
    ...
    _check(used.size.x > source_image.get_width() * 0.30, "Diego fighter became implausibly narrow")
    _check(used.size.y > source_image.get_height() * 0.70, "Diego fighter lost full-body height")
    ...
    _check(fighter != null, "Diego fighter texture failed to resolve")
    if fighter == null:
        return
    _check(VisualAssetCatalog.texture(path) == fighter, "Diego source texture is not authoritative in the path cache")

    var fighter_image := fighter.get_image()
    _check(fighter_image != null, "Diego fighter image is unavailable")
    if fighter_image != null:
        _check(fighter_image.get_size() == source_image.get_size(), "Diego runtime texture changed source dimensions")
        _check(_count_diego_guide_pixels(fighter_image) == 0, "Diego runtime texture reintroduced guide pixels")
        _check(_count_diego_face_artifact_pixels(fighter_image) <= 8, "Diego runtime texture reintroduced the facial square artifact")

    var portrait := VisualAssetCatalog.identity_portrait_texture_for_name(DIEGO_NAME)
    _check(portrait is AtlasTexture, "Diego portrait is not derived from the live fighter texture")
    if portrait is AtlasTexture:
        _check((portrait as AtlasTexture).atlas == fighter, "Diego portrait/live identity diverged")
...
func _count_diego_face_artifact_pixels(image: Image) -> int:
    var x_start := int(round(image.get_width() * 0.3667))
    var x_end := int(round(image.get_width() * 0.4750))
    var y_start := int(round(image.get_height() * 0.1286))
    var y_end := int(round(image.get_height() * 0.1750))
    var count := 0
    for y in range(y_start, y_end):
        for x in range(x_start, x_end):
            var pixel := image.get_pixel(x, y)
            if pixel.a > 0.5 and maxf(pixel.r, maxf(pixel.g, pixel.b)) < 0.28:
                count += 1
    return count
