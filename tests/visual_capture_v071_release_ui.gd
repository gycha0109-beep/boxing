...
func _run() -> void:
    ...
    if not bool(selected.get("ok", false)):
        ...
        return
    main_view._render_phase()
    await _capture("release_04_fight_opening.png")
    main_view._choose_fight_action("jab")
    await create_timer(0.08).timeout
    await _capture("release_05_fight_jab_contact.png")
    await create_timer(0.14).timeout
    await _capture("release_06_fight_jab_recovery.png")
    print("visual-capture-v071-release-ui: PASS")
    quit(0)
