...
func _run() -> void:
    ...
    _check(str(main_view.get_script().resource_path) == "res://scripts/main_v14.gd", "Main scene is not using the active economy shell that preserves the v1.0/v0.8/v0.6 device runtime")
