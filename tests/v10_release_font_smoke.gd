...
func _run() -> void:
    ...
    _check(str(main_view.get_script().resource_path) == "res://scripts/main_v16.gd", "Main scene is not using the active v16 UI shell that preserves the bundled-font runtime")
